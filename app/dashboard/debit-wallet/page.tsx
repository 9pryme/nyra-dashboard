...
import { AlertCircle, CheckCircle2, Search, ChevronDown, ReceiptText } from "lucide-react";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.data.success) {
        setFormSuccess("Wallet debited successfully");
        toast({
          title: "Success",
          description: "Wallet has been debited successfully",
        });
      ...
      if (axios.isAxiosError(err)) {
        setFormError(err.response?.data?.message || 'Failed to debit wallet');
      } else {
        setFormError(err instanceof Error ? err.message : 'An error occurred');
      }
      ...
      toast({
        title: "Error",
        description: err instanceof Error ? err.message : 'Failed to debit wallet',
        variant: "destructive",
      });
    } finally {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      >
        ...
        <p className="text-muted-foreground">Debit a user's wallet and view transaction history.</p>
      </motion.div>
